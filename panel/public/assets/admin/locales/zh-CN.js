...
window.XBOARD_TRANSLATIONS['zh-CN'] = {
  ...
  "settings": {
    ...
    "telegram": {
      "title": "Telegram设置",
      "description": "配置 Telegram 机器人、管理员通知和用户绑定功能。",
      "bot_config": {
        "title": "Bot 配置",
        "description": "配置 Telegram Bot Token 和 Webhook。"
      },
      "bot_token": {
        "title": "机器人令牌",
      ...
      "admin_notify": {
        "title": "管理员通知",
        "bound": "已检测到管理员账号绑定 Telegram，可选择需要接收的通知。",
        "unbound": "当前没有管理员账号绑定 Telegram，请先绑定后再开启通知。",
        "register": {
          "title": "新用户注册通知",
          "description": "新用户注册成功后向已绑定 Telegram 的管理员发送通知。"
        },
        "new_order": {
          "title": "新下单通知",
          "description": "新购套餐下单后发送套餐、原价、实付和优惠码信息。"
        },
        "renewal": {
          "title": "续费通知",
          "description": "套餐续费下单后发送与新订单相同的价格和优惠信息。"
        }
      },
      "user_features": {
        "title": "用户 Telegram 功能",
        "description": "控制用户端 Telegram 绑定引导及群组入口。"
      },
      "bot_enable": {
  ...
  "auth": {
    "signIn": {
      ...
      "resetPassword": {
        ...
        "command": "php artisan reset:password 管理员账号"
      },
      "validation": {
        "emailRequired": "请输入账号",
        "emailInvalid": "账号格式不正确",
        "passwordRequired": "请输入密码",
        "passwordLength": "请输入密码"
      }
    }
  },
  "traffic": {
  ...
  "server": {
    ...
    "dynamic_form": {
      ...
      "routing": {
        "outbounds_tab": "路由规则",
        "routes_tab": "出站规则",
        "outbounds_title": "选择出站规则",
        "outbounds_desc": "从出站规则管理中选择本节点需要使用的出口，保存后由后端自动转换为节点出站配置。",
        "outbounds_placeholder": "选择已添加的出口节点",
        "outbounds_empty": "暂无可用出站，请先到「出站规则管理」添加。",
        "refresh_outbounds": "刷新",
        "rules_title": "路由规则",
        "rules_desc": "按顺序匹配流量，可选择直连、阻断或转发到本节点已启用的出站。所有匹配条件均留空时表示全部流量。",
        "source": "来源",
        "network": "网络",
        "destination": "目标地址",
        "domain": "域名",
        ...
        "disabled": "已停用",
        "no_rules": "暂无路由规则",
        "node_scope_hint": "这些路由规则只保存在当前节点中，不会影响其他节点。",
        "rule_editor_desc": "逐条配置当前节点路由规则，字段逻辑参考 Dui/x-ui；匹配条件全部为空时表示全部流量。",
        "catch_all_hint": "如果源/目标 IP、端口、域名、网络类型和传输协议全部留空，则此规则匹配当前节点的全部 TCP/UDP 流量。",
        "rule_name": "规则名称",
        "rule_enabled": "启用规则",
        "node_only": "默认只对当前节点生效",
        "domains": "精确域名",
        "domain_suffixes": "域名后缀",
        "domain_address": "域地址",
        "ip_domain_relation": "IP / 域名关系",
        "ip_cidrs": "IP / CIDR",
        "ports": "端口",
        "source_cidrs": "源 IP",
        "source_ports": "源端口",
        ...
        "action_route": "指定出站",
        "target": "目标出站",
        "target_placeholder": "选择出口",
        "action_balancer": "负载均衡",
        "domain_advanced": "高级",
        "domain_advanced_title": "域地址高级规则",
        "domain_advanced_desc": "手动高级匹配或从 blackmatrix7 / ios_rule_script 自动识别导入。",
        "domain_manual": "手动高级",
        "domain_ruleset": "第三方应用分流",
        "no_target_port": "规则只作用于当前节点，不额外限制目标端口",
        "section": {
          "basic": "概要",
          "source": "来源",
          "network": "网络",
        ...
        "balancer": {
          "title": "负载均衡组",
          "description": "从当前节点已选择的出站中创建负载均衡组，路由规则可直接引用。",
          "add": "添加负载均衡",
          "strategy": "策略",
          "members": "成员",
          "empty": "暂无负载均衡组",
          "select": "负载均衡组",
          "xray_note": "负载均衡由双内核通用调度层执行。最低负载按当前活跃连接数选择；所有出口不可用时不会自动直连。",
          "tag_required": "请输入负载均衡 Tag",
          "tag_invalid": "Tag 只能包含字母、数字、点、下划线和连字符",
          "tag_duplicate": "负载均衡 Tag 已存在",
          "tag_conflict": "Tag 不能与出站 Tag 重复",
          "selector_required": "至少选择一个出站成员",
          "selector_invalid": "成员必须来自当前节点选择的出站",
          "fallback_invalid": "Fallback 必须来自当前节点选择的出站",
          "in_use": "该负载均衡组正在被路由规则使用"
        },
        "ruleset": {
          "select_required": "请先选择应用规则",
          "url_required": "请输入公开 List URL",
          "import_success": "规则已加入当前路由"
        },
        "error": {
          "target_required": "指定出站动作必须选择当前节点已启用的出站规则",
          "balancer_required": "负载均衡动作必须选择当前节点已创建的负载均衡组"
        }
      },
      "advanced": {
        "trigger_label": "高级设置",
  ...
  "route": {
    "title": "出站规则管理",
    "description": "统一管理第三方出口节点，可被多个节点复用。支持自动识别 VMess、VLESS、Trojan、Shadowsocks 节点链接。",
    "loading": "正在加载...",
    "empty": "暂无出站规则",
    "columns": {
      "id": "ID",
    ...
    "form": {
      ...
      "edit": "编辑出站规则",
      "link": "第三方节点链接",
      "linkRequired": "请输入第三方节点链接",
      "linkPlaceholder": "vmess://  vless://  trojan://  ss://",
      "linkHelp": "粘贴节点链接后可先点击识别，保存时后端也会自动识别。",
      "parse": "识别节点",
      "parsing": "识别中...",
      "detected": "识别结果",
      "name": "名称",
      "namePlaceholder": "例如：香港出口-01",
      "tag": "出站标签",
      "tagPlaceholder": "例如：hk-out-01",
      "remarks": "备注",
      "remarksPlaceholder": "可选备注",
      "enabled": "启用出站",
      "enabledHelp": "停用后引用该出口的节点不会下发此出站。",
      "submit": "保存"
    },
    "toolbar": {
      "searchPlaceholder": "搜索名称、标签或协议..."
    },
    "messages": {
      "deleteConfirm": "确认删除出站规则",
      "deleteDescription": "已被节点引用的出站规则不能删除。删除后不可恢复。",
      "deleteButton": "删除",
      "deleteSuccess": "删除成功",
      "createSuccess": "创建成功",
      "updateSuccess": "更新成功"
