...
class Plugin extends AbstractPlugin
{
  ...
  public function boot(): void
  {
    ...
    $this->listen('order.create.after', [$this, 'sendOrderCreateNotify'], 10);
  }


  public function sendRegisterNotify(User $user): void
  {
    if (!(bool) admin_setting('telegram_notify_register', false)) {
      return;
    }

    $message = "【新用户注册】\n";
    $message .= "用户：" . Helper::escapeMarkdown($user->email) . "\n";
    $message .= "时间：" . date('Y-m-d H:i:s');

    $this->telegramService->sendMessageWithAdmin($message);
  }

  public function sendOrderCreateNotify(Order $order): void
  {
    $type = (int) $order->type;

    if ($type === Order::TYPE_NEW_PURCHASE) {
      if (!(bool) admin_setting('telegram_notify_new_order', false)) {
        return;
      }
      $title = '【新订单】';
    } elseif ($type === Order::TYPE_RENEWAL) {
      if (!(bool) admin_setting('telegram_notify_renewal', false)) {
        return;
      }
      $title = '【续费订单】';
    } else {
    ...
    if (!$user || !$plan) {
      Log::warning('Telegram 订单通知失败：用户或套餐不存在', [
        'order_id' => $order->id,
    ...
    $planPrice = $plan->getPriceByPeriod((string) $order->period);
    $originalAmount = (int) round(((float) ($planPrice ?? 0)) * 100);
    $paidAmount = max(0, (int) $order->total_amount + (int) ($order->balance_amount ?? 0));

    $message = $title . "\n";
    $message .= "用户：" . Helper::escapeMarkdown($user->email) . "\n";
    $message .= "套餐：" . Helper::escapeMarkdown($plan->name) . "\n";
    $message .= "价格：" . $currencySymbol . $this->formatMoney($originalAmount) . " 原价\n";
    $message .= "实付：" . $currencySymbol . $this->formatMoney($paidAmount) . "\n";

    if ($order->coupon_id) {
      $coupon = Coupon::find($order->coupon_id);
      if ($coupon) {
        $message .= "优惠码：" . Helper::escapeMarkdown($coupon->code);
        $reason = $this->formatCouponReason($coupon, $currencySymbol);
        if ($reason !== '') {
          $message .= "（" . Helper::escapeMarkdown($reason) . "）";
        }
        $message .= "\n";
      }
    }
  ...
  private function formatCouponReason(Coupon $coupon, string $currencySymbol): string
  {
    $name = trim((string) $coupon->name);

    $discount = match ((int) $coupon->type) {
      1 => '减免 ' . $currencySymbol . $this->formatMoney((int) $coupon->value),
      2 => '减免 ' . rtrim(rtrim(number_format((float) $coupon->value, 2, '.', ''), '0'), '.') . '%',
      default => '',
    };
  ...
  {
    $welcomeTitle = $this->getConfig('start_welcome_title', '🎉 欢迎使用 DBoard Telegram Bot！');
    ...
    if ($user) {
      ...
      $welcomeText .= $this->getConfig('start_bind_guide', '🔗 请先绑定您的 DBoard 账号：\\n1. 登录您的 DBoard 账户\\n2. 复制您的订阅链接\\n3. 发送 /bind + 订阅链接') . "\n\n";
