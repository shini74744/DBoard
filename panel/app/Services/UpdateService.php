...
class UpdateService
{
    ...
    const GITHUB_API_URL = 'https://api.github.com/repos/shini74744/DBoard/commits';
    ...
    protected function pullLatestCode(): array
    {
        try {
            ...
            // DBoard is a monorepo. Running Git from panel/ is valid because
            // Git resolves the repository root from the parent directory.
            $gitCheck = Process::run('git rev-parse --is-inside-work-tree');
            if (!$gitCheck->successful() || trim($gitCheck->output()) !== 'true') {
                return [
                    'success' => false,
                    'message' => '当前部署不是 Git 源码检出，请使用 DBoard 发布包升级'
                ];
            }

            $fetch = Process::run('git fetch origin main');
            if (!$fetch->successful()) {
                return ['success' => false, 'message' => trim($fetch->errorOutput()) ?: 'git fetch failed'];
            }
            $reset = Process::run('git reset --hard origin/main');
            if (!$reset->successful()) {
                return ['success' => false, 'message' => trim($reset->errorOutput()) ?: 'git reset failed'];
            }

            // Update panel dependencies from panel/.
            $composer = Process::run('composer install --no-dev --optimize-autoloader');
            if (!$composer->successful()) {
                return ['success' => false, 'message' => trim($composer->errorOutput()) ?: 'composer install failed'];
            }
