...
func doTaskWithSnapshot(
	...
	if embeddedMode && (task.GetType() == model.TaskTypeApplyConfig || task.GetType() == model.TaskTypeServerTransferApply || task.GetType() == model.TaskTypeUpgrade) {
		result.Data = "Managed by the integrated control plane"
		return result
	}
