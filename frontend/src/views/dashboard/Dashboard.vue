        ...
        <template v-else>
          ...
          <div class="stats-card daily-usage-trigger usage-gradient-card"
              ...
              'permanent-card': displayedPermanent
               ...
               @keydown.space.prevent="showDailyTraffic = true">
            <div class="stats-icon">
            ...
            <IconChevronRight class="daily-usage-chevron" :size="20"/>
          </div>
          <DailyTrafficDialog v-if="showDailyTraffic" @close="showDailyTraffic = false"/>
...
export default {
  ...
  setup() {
    ...
    const isExpiringSoon = computed(() => {
      ...
      return Number.isFinite(days) && days > 0 && days <= DASHBOARD_CONFIG.expiringThreshold;
    });
    ...
    const isLowTraffic = computed(() => {
      if (trafficBytes.value.total <= 0 || trafficBytes.value.remaining <= 0) return false;
      return trafficBytes.value.remaining / trafficBytes.value.total * 100 <= DASHBOARD_CONFIG.lowTrafficThreshold;
    });

    const isTrafficDepleted = computed(() =>
      trafficBytes.value.total > 0 && trafficBytes.value.remaining <= 0
    );
    ...
    const fetchSubscribe = async (force = false) => {
      ...
      try {
        ...
        if (response.data) {
          ...
          const total = Math.max(0, Number(subscribe.transfer_enable) || 0);
          const used = Math.max(0, (Number(subscribe.u) || 0) + (Number(subscribe.d) || 0));
    ...
    onActivated(() => {
      ...
      createTimer(timers, 'subscriptionRefresh', refreshSubscription, 60000, true);
...
.daily-usage-chevron { margin-left: auto; flex: none; color: var(--secondary-text-color); }
