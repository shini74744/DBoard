...
export default {
  ...
  setup() {
    ...
    const { applyTheme, applyBrandTheme } = useTheme();
    let brandRefreshTimer;
    ...
    const refreshBrandTheme = async () => {
      try {
        const response = await request({ url: '/guest/comm/config', method: 'get', params: { _t: Date.now() } });
        applyBrandTheme(response?.data?.frontend_theme);
      } catch (error) {
        console.warn('Unable to refresh DBoard theme', error);
      }
    };

    const handleVisibilityChange = () => {
      if (!document.hidden) refreshBrandTheme();
      if (!document.hidden) {
    ...
    onMounted(() => {
      ...
      refreshBrandTheme();
      window.addEventListener('focus', refreshBrandTheme);
      brandRefreshTimer = window.setInterval(refreshBrandTheme, 60000);
    ...
    onUnmounted(() => {
      ...
      window.removeEventListener('focus', refreshBrandTheme);
