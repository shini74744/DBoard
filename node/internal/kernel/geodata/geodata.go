...
func atomicDownload(dst, url string) error {
	...
	// Multiple nodes can request the same dataset concurrently. A fixed .tmp
	// filename lets one writer truncate or rename another writer's download.
	f, err := os.CreateTemp(filepath.Dir(dst), ".geodata-*.tmp")
	if err != nil {
	...
	if n == 0 || n > maxDownloadBytes {
		f.Close()
		return fmt.Errorf("geodata download size out of bounds: %d", n)
	}
