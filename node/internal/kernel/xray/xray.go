...
func (x *Xray) Start(nodeConfig *model.NodeSpec, users []model.UserSpec, tls kernel.TLSCert) error {
	...
	prepared, err := kernel.PrepareStructuredRoutes(nodeConfig, x.cfg)
	if err != nil {
		return fmt.Errorf("prepare routes: %w", err)
	}
	x.ensureGeoData(prepared)

	data, err := marshalConfig(x.cfg, prepared, users, tls)
	if err != nil {
		return err
	}
...
func (x *Xray) Reload(nodeConfig *model.NodeSpec, users []model.UserSpec, tls kernel.TLSCert) error {
	...
	prepared, err := kernel.PrepareStructuredRoutes(nodeConfig, x.cfg)
	if err != nil {
		return fmt.Errorf("prepare candidate routes: %w", err)
	}
	candidateRouteHash := preparedRouteHash(prepared)
	x.updateDispatcherLimits(users)
	x.updateBandwidthLimits(users)
	...
	x.mu.Lock()
	same := x.lastKernelHash == newHash && x.lastPreparedRouteHash == candidateRouteHash && tlsEqual(x.tls, tls)
	if same {
		x.users = users
		x.mu.Unlock()
...
func preparedRouteHash(n *model.NodeSpec) string {
	if n == nil || len(n.CustomRouteRules) == 0 {
		return ""
	}
