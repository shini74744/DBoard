...
func TestBuildRouting_WithCustomRouteRules(t *testing.T) {
	...
	rules := routing["rules"].([]M)
	if len(rules) < 1 {
		t.Fatal("expected at least one compiled custom rule")
	}
	rule := rules[0]
	if rule["outboundTag"] != "warp-jp" {
		t.Fatalf("expected outbound warp-jp, got %v", rule["outboundTag"])
	}
	if got := rule["domain"].([]string); len(got) != 2 || got[0] != "full:full.example.com" || got[1] != "domain:example.org" {
		t.Fatalf("unexpected domains: %v", got)
	}
	if got := rule["ip"].([]string); len(got) != 1 || got[0] != "1.1.1.0/24" {
		t.Fatalf("unexpected ip: %v", got)
	}
	if got := rule["port"]; got != "80,443-445" {
		t.Fatalf("unexpected port matcher: %v", got)
	}
	if got := rule["network"]; got != "tcp,udp" {
		t.Fatalf("unexpected network matcher: %v", got)
	}
	if got := rule["protocol"].([]string); len(got) != 2 || got[0] != "http" || got[1] != "tls" {
		t.Fatalf("unexpected protocol matcher: %v", got)
	}
	if got := rule["source"].([]string); len(got) != 1 || got[0] != "192.168.1.0/24" {
		t.Fatalf("unexpected source matcher: %v", got)
	}
	if got := rule["sourcePort"]; got != "1000-1002" {
		t.Fatalf("unexpected source port matcher: %v", got)
	}
}

func TestBuildRouting_IPDomainOrSplitsIntoTwoRules(t *testing.T) {
	custom := []model.CustomRouteRule{{
		IPDomainRelation: "or",
		Match: model.RouteMatch{
			DomainSuffixes: []string{"example.com"},
			IPCIDRs:        []string{"8.8.8.8/32"},
			Ports:          []string{"443"},
		},
		Action: model.RouteAction{Type: "route", Target: "proxy"},
	}}
	routing := buildRouting(nil, custom, nil)
	rules := routing["rules"].([]M)
	if len(rules) < 2 {
		t.Fatalf("expected two OR branches, got %d", len(rules))
	}
	if _, ok := rules[0]["domain"]; !ok {
		t.Fatalf("expected domain branch first: %#v", rules[0])
	}
	if _, ok := rules[0]["ip"]; ok {
		t.Fatalf("domain branch should not include ip: %#v", rules[0])
	}
	if _, ok := rules[1]["ip"]; !ok {
		t.Fatalf("expected ip branch second: %#v", rules[1])
	}
	if rules[0]["port"] != "443" || rules[1]["port"] != "443" {
		t.Fatalf("common port must be copied to both OR branches: %#v %#v", rules[0], rules[1])
	}
}

func TestBuildRouting_CustomRouteCatchAll(t *testing.T) {
	custom := []model.CustomRouteRule{{
		Name:   "all-traffic",
		Match:  model.RouteMatch{},
	...
	if rules[0]["type"] != "field" {
		t.Fatalf("unexpected catch-all type: %v", rules[0]["type"])
	}
	if rules[0]["network"] != "tcp,udp" {
		t.Fatalf("unexpected catch-all network: %v", rules[0]["network"])
	}
	if rules[0]["outboundTag"] != "proxy-all" {
		t.Fatalf("unexpected catch-all outbound: %v", rules[0]["outboundTag"])
	}
}
