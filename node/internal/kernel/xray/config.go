...
func buildConfig(kcfg config.KernelConfig, nc *model.NodeSpec, users []model.UserSpec, tc kernel.TLSCert) M {
	...
	// Panel-defined custom outbounds.
	// Share-link imports from our Xboard fork use a normalized cross-kernel shape;
	// convert those into native Xray vnext/servers + streamSettings here.
	for _, co := range nc.CustomOutbounds {
		if converted, ok := structuredProxyOutboundToXray(co); ok {
	...
	cfg["routing"] = routing
	// Managed groups use the shared scheduler. Native raw Balancer JSON remains
	// an explicit escape hatch and is not rewritten.
	for _, b := range nc.CustomBalancers {
		outbounds = append(outbounds, M{"tag": b.Tag, "protocol": "blackhole", "settings": M{}})
	}
...
func buildXrayBalancers(items []model.CustomBalancer) []M {
	if len(items) == 0 {
		return nil
	}
	...
	for _, item := range items {
		...
		if tag == "" || len(item.Selector) == 0 {
			continue
		}
		b := M{
			"tag":      tag,
			"selector": copyStrings(item.Selector),
		}
		strategy := strings.TrimSpace(item.Strategy)
		if strategy != "" && strategy != "random" {
			b["strategy"] = M{"type": strategy}
		}
		if fallback := strings.TrimSpace(item.FallbackTag); fallback != "" {
...
func applyXrayBalancerObservatory(cfg M, items []model.CustomBalancer) {
	leastPing := map[string]bool{}
	...
	if len(leastPing) > 0 {
		cfg["observatory"] = M{
			"subjectSelector":   mapKeysSorted(leastPing),
	...
	if len(burst) > 0 {
		cfg["burstObservatory"] = M{
			"subjectSelector": mapKeysSorted(burst),
			"pingConfig": M{
...
func buildInbound(nc *model.NodeSpec, users []model.UserSpec, tc kernel.TLSCert) M {
	...
	if needsUniversalSniff(nc) {
		base["sniffing"] = M{"enabled": true, "destOverride": []string{"http", "tls", "quic"}, "routeOnly": true}
	}
...
func compileCustomRouteRule(rule model.CustomRouteRule) []M {
	common := M{"type": "field"}
	for key, value := range xrayRouteActionFields(rule.Action) {
		common[key] = value
	}

	if len(rule.Match.Ports) > 0 {
		common["port"] = strings.Join(rule.Match.Ports, ",")
	}
	if len(rule.Match.Networks) > 0 {
		common["network"] = strings.Join(rule.Match.Networks, ",")
	}
	if len(rule.Match.Protocols) > 0 {
		common["protocol"] = copyStrings(rule.Match.Protocols)
	}
	if len(rule.Match.SourceCIDRs) > 0 {
		common["source"] = copyStrings(rule.Match.SourceCIDRs)
	}
	...
	for _, value := range rule.Match.Domains {
		...
		case strings.HasPrefix(value, "full:"),
			...
			strings.HasPrefix(value, "keyword:"),
			strings.HasPrefix(value, "regexp:"),
	...
	for _, value := range rule.Match.DomainSuffixes {
		...
		if strings.HasPrefix(value, "domain:") || strings.HasPrefix(value, "geosite:") {
			domains = append(domains, value)
		} else {
	...
	ips := copyStrings(rule.Match.IPCIDRs)
	hasDomain := len(domains) > 0
	hasIP := len(ips) > 0
	hasCommon := len(common) > 2
	...
	if !hasDomain && !hasIP && !hasCommon {
		catchAll := cloneRule()
		catchAll["network"] = "tcp,udp"
		return []M{catchAll}
	}

	relation := strings.ToLower(strings.TrimSpace(rule.IPDomainRelation))
	if hasDomain && hasIP && relation != "and" {
		domainRule := cloneRule()
		domainRule["domain"] = domains
		ipRule := cloneRule()
		ipRule["ip"] = ips
		return []M{domainRule, ipRule}
	}

	compiled := cloneRule()
	if hasDomain {
		compiled["domain"] = domains
	}
	if hasIP {
		compiled["ip"] = ips
	}
...
func xrayRouteActionFields(action model.RouteAction) M {
	switch strings.ToLower(strings.TrimSpace(action.Type)) {
	case "direct":
		return M{"outboundTag": "direct"}
	case "route":
		if target := strings.TrimSpace(action.Target); target != "" {
			return M{"outboundTag": target}
		}
		return M{"outboundTag": "block"}
	case "balancer":
		if target := strings.TrimSpace(action.Target); target != "" {
			return M{"outboundTag": target}
		}
