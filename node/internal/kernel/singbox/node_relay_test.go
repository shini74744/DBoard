...
func TestNodeRelayDedicatedIdentity(t *testing.T) {
	...
	exit := New(config.KernelConfig{Type: "singbox", LogLevel: "error"})
	...
	defer exit.Stop()
	entry := New(config.KernelConfig{Type: "singbox", LogLevel: "error"})
...
func TestNodeRelayDisabledExitConfiguration(t *testing.T) {
	...
	l.Close()
	instance := New(config.KernelConfig{Type: "singbox", LogLevel: "error"})
