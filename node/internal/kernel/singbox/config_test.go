...
func TestBuildConfig(t *testing.T) {
	...
	if len(outbounds) != 1 {
		t.Fatalf("removed block outbound should not be generated: %v", outbounds)
	}
	assertMapValue(t, outbounds[0], "type", "direct")
	// Blocking is expressed with action: reject, not the removed block outbound.
...
func TestBuildRoutes_WithCustomRouteRules(t *testing.T) {
	rule := model.CustomRouteRule{IPDomainRelation: "and", Match: model.RouteMatch{Domains: []string{"full:full.example.com", "keyword:example", "regexp:^test"}, DomainSuffixes: []string{"example.org"}, IPCIDRs: []string{"1.1.1.0/24"}, Ports: []string{"53", "1000-1002"}, Networks: []string{"tcp"}, Protocols: []string{"tls"}, SourceCIDRs: []string{"10.0.0.0/8"}, SourcePorts: []string{"2000-2001"}}, Action: model.RouteAction{Type: "direct"}}
	rules := compileCustomRouteRule(rule)
	if len(rules) != 1 || rules[0]["type"] != "logical" || rules[0]["mode"] != "and" {
		t.Fatalf("incorrect logical root %#v", rules)
	}
	children := rules[0]["rules"].([]M)
	if len(children) != 6 {
		t.Fatalf("matcher families were flattened: %#v", children)
	}
	dest := children[5]
	if dest["mode"] != "and" || len(dest["rules"].([]M)) != 2 {
		t.Fatalf("missing destination AND: %#v", dest)
	}
	domains := dest["rules"].([]M)[0]
	if domains["domain"].([]string)[0] != "full.example.com" || domains["domain_keyword"].([]string)[0] != "example" {
		t.Fatalf("prefix conversion failed %#v", domains)
	}
}
func TestBuildRoutes_IPDomainOrSplitsIntoTwoRules(t *testing.T) {
	rule := model.CustomRouteRule{IPDomainRelation: "or", Match: model.RouteMatch{DomainSuffixes: []string{"example.com"}, IPCIDRs: []string{"8.8.8.8/32"}, Ports: []string{"443"}}, Action: model.RouteAction{Type: "route", Target: "proxy"}}
	rules := compileCustomRouteRule(rule)
	if len(rules) != 1 || rules[0]["mode"] != "and" {
		t.Fatalf("missing common AND %#v", rules)
	}
	children := rules[0]["rules"].([]M)
	if children[0]["port"].([]int)[0] != 443 || children[1]["mode"] != "or" {
		t.Fatalf("port must AND with address OR: %#v", children)
	}
}

func TestBuildRoutes_CustomRouteCatchAll(t *testing.T) {
	custom := []model.CustomRouteRule{{
		Name:   "all-traffic",
		Match:  model.RouteMatch{},
		Action: model.RouteAction{Type: "route", Target: "proxy-all"},
	}}
	route := buildRoutes(nil, custom, nil)
	rules := route["rules"].([]M)
	if len(rules) < 1 {
		t.Fatal("expected catch-all rule")
	}
	networks, ok := rules[0]["network"].([]string)
	if !ok || len(networks) != 2 || networks[0] != "tcp" || networks[1] != "udp" {
		t.Fatalf("unexpected catch-all network: %#v", rules[0]["network"])
	}
	if rules[0]["outbound"] != "proxy-all" {
		t.Fatalf("unexpected catch-all outbound: %v", rules[0]["outbound"])
	}
}
