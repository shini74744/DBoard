...
func buildConfig(kcfg config.KernelConfig, nc *model.NodeSpec, users []model.UserSpec, tc kernel.TLSCert) M {
	...
	// Share-link imports from our Xboard fork use a normalized shape;
	// convert those into native sing-box fields here.
	for _, co := range nc.CustomOutbounds {
		if converted, ok := structuredProxyOutboundToSingbox(co); ok {
	...
	for _, b := range nc.CustomBalancers {
		outbounds = append(outbounds, M{"type": universalBalancerType, "tag": b.Tag,
			"strategy": b.Strategy, "members": b.Selector, "fallback": b.FallbackTag, "probe_url": b.ProbeURL,
			"probe_interval_seconds": b.ProbeIntervalSeconds, "probe_timeout_seconds": b.ProbeTimeoutSeconds})
...
// Every matcher family is a separate AND child. sing-box default rules group
...
func compileCustomRouteRule(rule model.CustomRouteRule) []M {
	...
	if len(rule.Match.Ports) > 0 {
		p, r := splitPorts(rule.Match.Ports)
		v := M{}
		if len(p) > 0 {
			v["port"] = p
		}
	...
	if len(rule.Match.Networks) > 0 {
		common = append(common, M{"network": copyStrings(rule.Match.Networks)})
	}
	if len(rule.Match.Protocols) > 0 {
		common = append(common, M{"protocol": copyStrings(rule.Match.Protocols)})
	}
	...
	add := func(key, value string) {
		if value != "" {
			old, _ := d[key].([]string)
			d[key] = append(old, value)
		}
	}
	for _, raw := range rule.Match.Domains {
		v := strings.TrimSpace(raw)
		switch {
		case strings.HasPrefix(v, "full:"):
			add("domain", strings.TrimPrefix(v, "full:"))
		case strings.HasPrefix(v, "domain:"):
			add("domain_suffix", strings.TrimPrefix(v, "domain:"))
		case strings.HasPrefix(v, "keyword:"):
			add("domain_keyword", strings.TrimPrefix(v, "keyword:"))
		case strings.HasPrefix(v, "regexp:"):
			add("domain_regex", strings.TrimPrefix(v, "regexp:"))
		default:
			add("domain", v)
		}
	}
	...
	if len(rule.Match.IPCIDRs) > 0 {
		destinations = append(destinations, M{"ip_cidr": copyStrings(rule.Match.IPCIDRs)})
	}
	if len(destinations) > 1 {
		mode := "or"
		if strings.EqualFold(rule.IPDomainRelation, "and") {
			mode = "and"
		}
		common = append(common, M{"type": "logical", "mode": mode, "rules": destinations})
	} else {
		common = append(common, destinations...)
	}
	var compiled M
	if len(common) == 0 {
		compiled = M{"network": []string{"tcp", "udp"}}
	} else if len(common) == 1 {
		compiled = common[0]
	} else {
		compiled = M{"type": "logical", "mode": "and", "rules": common}
	}
	if strings.EqualFold(rule.Action.Type, "block") {
		compiled["action"] = "reject"
	} else {
		compiled["action"] = "route"
		compiled["outbound"] = singboxOutboundForAction(rule.Action)
	}
	return []M{compiled}
}
