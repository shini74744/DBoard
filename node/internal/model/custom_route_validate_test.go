...
func TestValidateCustomRouteRules(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			kernel:  "singbox",
			wantErr: "custom_route_rules[0].action.target is only allowed when action.type is route or balancer",
		},
		{
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			err := ValidateCustomRouteRules(tc.rules, tc.kernel, available, map[string]struct{}{"bal": {}})
