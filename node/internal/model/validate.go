...
func ValidateNodeSpec(n *NodeSpec, kcfg config.KernelConfig) error {
	...
	if err := validateUniversalRuleValues(n.CustomRouteRules); err != nil {
		return err
	}
	...
	balancerTags, err := ValidateCustomBalancers(n.CustomBalancers, kernelType, availableTags)
	if err != nil {
		return fmt.Errorf("validate custom balancers: %w", err)
	}
	if err := ValidateCustomRouteRules(n.CustomRouteRules, kernelType, availableTags, balancerTags); err != nil {
		return fmt.Errorf("validate custom route rules: %w", err)
	}
	if err := validateUniversalReferences(n, additionalTags); err != nil {
		return err
	}
