...
func NodeSpecFromPanel(nc *panel.NodeConfig) *NodeSpec {
	...
	customBalancers := make([]CustomBalancer, 0, len(nc.CustomBalancers))
	for _, balancer := range nc.CustomBalancers {
		customBalancers = append(customBalancers, CustomBalancer{
			Tag:                  balancer.Tag,
			Strategy:             balancer.Strategy,
			Selector:             cloneStringSlice(balancer.Selector),
			FallbackTag:          balancer.FallbackTag,
			ProbeURL:             balancer.ProbeURL,
			ProbeIntervalSeconds: balancer.ProbeIntervalSeconds,
			ProbeTimeoutSeconds:  balancer.ProbeTimeoutSeconds,
		})
	}
	...
	result := &NodeSpec{
		...
		CustomBalancers:     customBalancers,
		CertConfig:          certCfg,
		AutoTLS:             nc.AutoTLS,
		Domain:              nc.Domain,
		Cipher:              nc.Cipher,
...
func (n *NodeSpec) ToPanel() *panel.NodeConfig {
	...
	customBalancers := make([]panel.CustomBalancer, 0, len(n.CustomBalancers))
	for _, balancer := range n.CustomBalancers {
		customBalancers = append(customBalancers, panel.CustomBalancer{
			Tag:                  balancer.Tag,
			Strategy:             balancer.Strategy,
			Selector:             cloneStringSlice(balancer.Selector),
			FallbackTag:          balancer.FallbackTag,
