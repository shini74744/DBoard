...
func ValidateCustomRouteRules(rules []CustomRouteRule, kernelType string, availableTags map[string]struct{}, balancerTags map[string]struct{}) error {
	if len(rules) == 0 {
		return nil
	}
	...
	for i, rule := range rules {
		...
		if err := validateProtocols(rule.Match.Protocols, fmt.Sprintf("custom_route_rules[%d].match.protocols", i)); err != nil {
			return err
		}
		...
		case "balancer":
			if target == "" {
				return fmt.Errorf("custom_route_rules[%d].action.target is required when action.type is balancer", i)
			}
			if balancerTags != nil {
				if _, ok := balancerTags[strings.ToLower(target)]; !ok {
					return fmt.Errorf("custom_route_rules[%d].action.target references unknown balancer %q", i, rule.Action.Target)
				}
			}
		case "direct", "block":
			if target != "" {
				return fmt.Errorf("custom_route_rules[%d].action.target is only allowed when action.type is route or balancer", i)
			}
		default:
...
func validateProtocols(values []string, field string) error {
	...
	for _, value := range values {
		...
		if !allowed[value] {
			return fmt.Errorf("%s contains unsupported protocol %q", field, value)
		}
	}
	return nil
}
